...
namespace KeyboardEventHandlers
{
    ...
    intptr_t HandleSingleKeyRemapEvent(KeyboardManagerInput::InputInterface& ii, LowlevelKeyboardEvent* data, State& state) noexcept
    {
        ...
        if (!GeneratedByKBM(data))
        {
            ...
            state.UpdateCopilotKeyState(data->lParam->vkCode, isKeyDown);
            const bool isInjected = (data->lParam->flags & LLKHF_INJECTED) != 0;

            // Check if this is the Copilot key sequence
            DWORD lookupKey = data->lParam->vkCode;
            bool isCopilotKey = false;
            if (data->lParam->vkCode == VK_F23 && (state.IsCopilotKeyActive() || (!isKeyDown && state.IsCopilotKeyReleasing())))
            {
                // Treat F23 as VK_COPILOT for remap lookup
                lookupKey = CommonSharedConstants::VK_COPILOT;
                isCopilotKey = true;
                Logger::trace(L"HandleSingleKeyRemapEvent: Copilot key active, using VK_COPILOT for lookup");
            }
            // Safety: Use robust suppression check that includes timeout validation
            else if (state.ShouldSuppressCopilotSequenceKey(data->lParam->vkCode, isKeyDown, isInjected))
            {
                // Check if Copilot key has a remap configured (either to key/shortcut or to text)
                const auto copilotRemap = state.GetSingleKeyRemap(CommonSharedConstants::VK_COPILOT);
                const auto copilotTextRemap = state.GetSingleKeyToTextRemapEvent(CommonSharedConstants::VK_COPILOT);

                if (copilotRemap || copilotTextRemap)
                {
                    Logger::trace(L"HandleSingleKeyRemapEvent: Suppressing Copilot sequence key vkCode=0x{:X}", data->lParam->vkCode);
                    return 1;
                }
            }

            UpdateNumpadWithShift(data, state);
            const auto remapping = state.GetSingleKeyRemap(lookupKey);
            if (remapping)
            {
                ...
                if (remapToKey)
                {
                    ...
                    {
                        // If this was the Copilot key, also suppress Win/Shift
                        return 1;
                    }
                }
    ...
    */
    ...
    intptr_t HandleSingleKeyToTextRemapEvent(KeyboardManagerInput::InputInterface& ii, LowlevelKeyboardEvent* data, State& state)
    {
        ...
        // Update Copilot key state machine
        bool isKeyDown = true; // This function only processes keydown events
        state.UpdateCopilotKeyState(data->lParam->vkCode, isKeyDown);
        const bool isInjected = (data->lParam->flags & LLKHF_INJECTED) != 0;

        // Check if this is the Copilot key sequence
        DWORD lookupKey = data->lParam->vkCode;
        if (data->lParam->vkCode == VK_F23 && state.IsCopilotKeyActive())
        {
            lookupKey = CommonSharedConstants::VK_COPILOT;
        }
        // Safety: Use robust suppression check
        else if (state.ShouldSuppressCopilotSequenceKey(data->lParam->vkCode, isKeyDown, isInjected))
        {
            // Check if Copilot key has a text remap configured
            const auto copilotTextRemap = state.GetSingleKeyToTextRemapEvent(CommonSharedConstants::VK_COPILOT);
            const auto copilotRemap = state.GetSingleKeyRemap(CommonSharedConstants::VK_COPILOT);

            if (copilotTextRemap || copilotRemap)
            {
                return 1;
            }
        }

        const auto remapping = state.GetSingleKeyToTextRemapEvent(lookupKey);
        if (!remapping)
        {
            return 0;
        }
